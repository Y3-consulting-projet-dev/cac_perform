    ...
    try:
        ...
        if hasattr(db_manager, "config") and db_manager.config is not None:
            try:
                ...
                raise RuntimeError(
                    f"Base de données non connectée: {e}"
    ...
    def afficher_missions(cls, id):
        ...
        if total_missions > 0 and count == 0:
            ...
            for sample in sample_missions:
                ...
                print(
                    f"   - Mission {sample['_id']}: id_client='{sample_id_client}' (type: {type(sample_id_client)})"
                )
...
class Mission(Document):
    ...
    def get_referentiel_groupes(self, referentiel="syscohada"):
        """
        Charge le mapping depuis mapping_simple.json
        Retourne un dict avec 'comptes', 'grands_groupes', 'groupings'
        """
        try:
            mapping_path = os.path.join(
                os.path.dirname(__file__), "..", "mapping_simple.json"
            )
            
            with open(mapping_path, "r", encoding="utf-8") as file:
                data = json.load(file)
            
            # Le nouveau mapping_simple.json a la structure:
            # {
            #   "comptes": {...},
            #   "grands_groupes": {...},
            #   "groupings": {...}
            ...
            print(f"✅ Mapping chargé : {len(data.get('comptes', {}))} comptes")
            return data
            
        except Exception as e:
            print(f"❌ Erreur chargement référentiel '{referentiel}': {str(e)}")
            return None
    ...
    def revue_analytique(self, id_mission):
            ...
            for key in (
                ...
                if key in bloc and bloc[key]:
                    if isinstance(bloc[key], str):
                        prefixes += [
                            p.strip() for p in bloc[key].split(",") if p.strip()
                        ]
                    elif isinstance(bloc[key], list):
                        prefixes += [
                            str(p).strip() for p in bloc[key] if str(p).strip()
                        ]
            return prefixes
        ...
        db.Mission1.update_one(
            {"_id": ObjectId(id_mission)}, {"$set": {"revue_analytique": out}}
        )
        return out
    ...
    def update_commentaire_perso(self, id_mission, numero_compte, commentaire_perso):
        ...
        try:
            ...
            index = next(
                (
                    i
                    for i, it in enumerate(revue)
                    if it.get("numero_compte") == numero_compte
                ),
                None,
            )
            if index is None:
            ...
            result = db.Mission1.update_one(
                {"_id": ObjectId(id_mission)}, {"$set": {path: commentaire_perso or ""}}
            )
    ...
    def update_commentaire_perso(self, id_mission, numero_compte, commentaire_perso):
        ...
        try:
            ...
            current_comment = revue_analytique[compte_index].get(
                "commentaire_perso", ""
            )
            if current_comment == commentaire_perso:
                print(
                    f"Commentaire identique pour le compte {numero_compte}, aucune modification nécessaire"
                )
                return True  # Retourner True car c'est un succès (pas d'erreur)

            print(
                f"Mise à jour du commentaire: '{current_comment}' -> '{commentaire_perso}'"
            )
            ...
            result = db.Mission1.update_one(
                ...
                {
                    "$set": {
                        f"revue_analytique.{compte_index}.commentaire_perso": commentaire_perso
                    }
                },
            )
            ...
            if success:
                print(
                    f"Commentaire mis à jour avec succès pour le compte {numero_compte}"
                )
            else:
                print(
                    f"Aucune modification effectuée pour le compte {numero_compte} (modified_count: {result.modified_count})"
                )
    ...
    def nouvelle_mission(
        ...
        try:
            ...
            if db is None:
                ...
                if db is None:
                    raise Exception(
                        "Base de données non connectée (get_db a renvoyé None)"
                    )
        except Exception as e:
            print(
                f"❌ ERREUR: Impossible d'obtenir la connexion à la base de données: {e}"
            )
        ...
        referentiel = (
            "syscohada"  # Vous pouvez le récupérer depuis les données si nécessaire
        )
        grouping_model = self.create_grouping(balance_variation, referentiel)
        etats = self.prod_efi(
            les_balance_n_n1[0], les_balance_n_n1[1], balance_variation
        )
        ...
        if mission_saved:
            print(
                f"✅ Mission vérifiée en base: id_client='{mission_saved.get('id_client')}'"
            )
        else:
    ...
    def creation_balance(self, balance_data, annee_auditee, id_client):
        ...
        try:
            ...
            for name in accepted_sheet_names:
                if name in workbook.sheetnames:
                    ...
                    sheet_name = name
                    print(
                        f"✅ Feuille trouvée: '{name}' dans {balance.filename if hasattr(balance, 'filename') else 'fichier'}"
                    )
                    break
            ...
            if sheet is None and workbook.sheetnames:
                ...
                print(
                    f"⚠️  Feuille non standard utilisée: '{sheet_name}' dans {balance.filename if hasattr(balance, 'filename') else 'fichier'}"
                )
            ...
            print(
                f"📊 Traitement de la feuille '{sheet_name}' avec {sheet.max_row} lignes et {sheet.max_column} colonnes"
            )
            ...
            formats_supportes = {
                "standard": {
                    ...
                    "detection": lambda row: len(row) >= 8
                    and any("compte" in str(cell).lower() for cell in row if cell),
                },
                "balance_simple": {
                    "pattern": [
                        "numero",
                    ...
                    "detection": lambda row: len(row) >= 6
                    and (
                        any("compte" in str(cell).lower() for cell in row if cell)
                        or (str(row[0]).isdigit() and len(str(row[0])) >= 3)
                    ),
                },
                "sage": {
                    "pattern": [
                ...
                for format_name in format_priority:
                    if format_name in formats_supportes:
                        ...
                        if format_info["detection"](row_data):
                            ...
                            print(
                                f"✅ Format détecté: {format_name} à la ligne {row_idx}"
                            )
                            break
            ...
            if format_detecte == "sage":
                ...
                for row_idx, row in enumerate(
                    ...
                    try:
                        ...
                        ligne["sign_solde"] = (
                            "D" if ligne["debit_fin"] >= ligne["credit_fin"] else "C"
                        )
                ...
                if lignes_ignorees_colonnes > 0:
                    print(
                        f"   ⚠️  Lignes ignorées (moins de 6 colonnes): {lignes_ignorees_colonnes}"
                    )
                if lignes_ignorees_vides > 0:
                    print(
                        f"   ⚠️  Lignes ignorées (numéro de compte vide): {lignes_ignorees_vides}"
                    )
                if lignes_ignorees_numero_invalide > 0:
                    print(
                        f"   ⚠️  Lignes ignorées (numéro de compte invalide, doit être numérique avec 3+ chiffres): {lignes_ignorees_numero_invalide}"
                    )
                if lignes_erreur > 0:
                ...
                for row_idx in range(1, min(5, sheet.max_row + 1)):
                    row_data = [
                        sheet.cell(row=row_idx, column=col).value
                        for col in range(1, min(9, sheet.max_column + 1))
                    ]
                ...
                if sheet.max_column >= 8:
                    ...
                    col7_header = str(
                        header_row_data[6] if len(header_row_data) > 6 else ""
                    ).strip()
                    col8_header = str(
                        header_row_data[7] if len(header_row_data) > 7 else ""
                    ).strip()
                    col7_val = col7_header.lower()
                    col8_val = col8_header.lower()

                    print(
                        f"📝 DÉTECTION DE L'ORDRE DES COLONNES (format balance_simple):"
                    )
                    print(f"   Colonne 7 (index 6): '{col7_header}'")
                    print(f"   Colonne 8 (index 7): '{col8_header}'")

                    has_solde_credit_col7 = (
                        ("solde" in col7_val and "credit" in col7_val)
                        or ("solde" in col7_val and "crédit" in col7_val)
                        or (
                            col7_val
                            and "credit" in col7_val
                            and "debit" not in col7_val
                            and "débit" not in col7_val
                        )
                    )

                    has_solde_debit_col8 = (
                        ("solde" in col8_val and "debit" in col8_val)
                        or ("solde" in col8_val and "débit" in col8_val)
                        or (
                            col8_val
                            and "debit" in col8_val
                            and "credit" not in col8_val
                            and "crédit" not in col8_val
                        )
                    )
                    ...
                    if has_solde_credit_col7 and has_solde_debit_col8:
                        ...
                        print(
                            f"✅ Format détecté: Solde crédit (col 7) puis Solde débit (col 8) - COLONNES INVERSÉES"
                        )
                    else:
                        print(
                            f"✅ Format détecté: Débit (col 7) puis Crédit (col 8) - FORMAT STANDARD"
                        )
                ...
                for row_idx, row in enumerate(
                    ...
                    try:
                        ...
                        if (
                            not num_compte_str
                            or num_compte_str.lower() == "none"
                            or num_compte_str.lower() == "nan"
                        ):
                            raise ValueError(
                                f"Numéro de compte invalide: '{num_compte_raw}'"
                            )
                        ...
                        try:
                            ...
                            if len(row) > 5 and (
                                ...
                                solde_initial = (
                                    ligne["debit_initial"] - ligne["credit_initial"]
                                )
                                ...
                                if mouvement_net > 0:
                                    ...
                                    debit_mvt_calc = max(
                                        0, ligne["debit_fin"] - ligne["debit_initial"]
                                    )
                                    credit_mvt_calc = max(
                                        0, ligne["credit_fin"] - ligne["credit_initial"]
                                    )
                ...
                if lignes_ignorees_numero_vide > 0:
                    print(
                        f"   ⚠️  Lignes ignorées (numéro de compte vide): {lignes_ignorees_numero_vide}"
                    )
                if lignes_ignorees_vides > 0:
                ...
                has_debit_col7 = (
                    "debit" in col7_val
                    and "credit" not in col7_val
                    and "crédit" not in col7_val
                ) or (
                    "débit" in col7_val
                    and "credit" not in col7_val
                    and "crédit" not in col7_val
                ...
                has_credit_col8 = (
                    "credit" in col8_val
                    and "debit" not in col8_val
                    and "débit" not in col8_val
                ) or (
                    "crédit" in col8_val
                    and "debit" not in col8_val
                    and "débit" not in col8_val
                ...
                if has_solde_credit_col7 and has_solde_debit_col8:
                    ...
                    print(
                        f"   Mapping: credit_fin = row[6] (col 7), debit_fin = row[7] (col 8)"
                    )
                    ...
                    print(
                        f"   Mapping: debit_fin = row[6] (col 7), credit_fin = row[7] (col 8)"
                    )
                else:
                    # Par défaut, utiliser le format standard
                    colonnes_inversees = False
                    print(
                        f"⚠️  Format non reconnu, utilisation du format standard par défaut"
                    )
                    print(
                        f"   Mapping: debit_fin = row[6] (col 7), credit_fin = row[7] (col 8)"
                    )
                ...
                if lignes_ignorees_colonnes > 0:
                    print(
                        f"   ⚠️  Lignes ignorées (moins de 8 colonnes): {lignes_ignorees_colonnes}"
                    )
                if lignes_ignorees_numero_vide > 0:
                    print(
                        f"   ⚠️  Lignes ignorées (numéro de compte vide): {lignes_ignorees_numero_vide}"
            ...
            print(
                f"✅ {len(data)} lignes de données extraites de la feuille '{sheet_name}'"
            )
        ...
        if len(data) == 0:
            filename = (
                balance.filename if hasattr(balance, "filename") else "fichier inconnu"
            )
            raise Exception(
                f"Aucune donnée valide trouvée dans le fichier '{filename}'. Vérifiez le format du fichier Excel."
            )
    ...
    def create_grouping(self, balances_rapprochee, referentiel="syscohada"):
        """
        Crée le grouping en utilisant mapping_simple.json (structure optimisée)
        
        Args:
            balances_rapprochee: Liste des comptes avec solde_n et solde_n1
            referentiel: Nom du référentiel (par défaut "syscohada")
        
        Returns:
            Liste des grands groupes avec ref, libellé, solde_n, solde_n1, variation
        """
        ...
        # =============================
        # 1️⃣ Charger le mapping (mapping_simple.json)
        # =============================
        try:
            mapping_data = self.get_referentiel_groupes(referentiel)
            
            if not mapping_data:
                raise ValueError(f"Impossible de charger le mapping pour '{referentiel}'")
            
            print(f"✅ Mapping chargé : {len(mapping_data.get('comptes', {}))} comptes")
        except Exception as e:
            print(f"❌ Erreur chargement mapping: {str(e)}")
            raise ValueError(f"Impossible de charger le mapping: {str(e)}")

        # Extraire les structures du mapping
        comptes_mapping = mapping_data.get("comptes", {})
        grands_groupes_mapping = mapping_data.get("grands_groupes", {})
        groupings_index = mapping_data.get("groupings", {})

        # =============================
        # 2️⃣ Initialiser les structures de grouping
        # =============================
        grouping_aggregation = {}

        # =============================
        # 3️⃣ Agréger les comptes par grouping
        # =============================
        comptes_non_mappes = []
        ...
        for item in balances_rapprochee:
            ...
            # Récupérer les métadonnées du compte depuis le mapping
            compte_info = comptes_mapping.get(numero)
            
            # Si le compte exact n'est pas trouvé, chercher par préfixe (2 premiers chiffres)
            if not compte_info and len(numero) >= 2:
                prefixe = numero[:2]
                # Chercher dans les comptes du grouping correspondant
                for compte_key, info in comptes_mapping.items():
                    if compte_key.startswith(prefixe):
                        compte_info = info
                        break
            
            # Si toujours pas trouvé, créer un grouping automatique
            if not compte_info:
                comptes_non_mappes.append(numero)
                # Créer un grouping temporaire basé sur la classe
                prefixe = numero[:1] if len(numero) >= 1 else "0"
                grouping_num = int(prefixe) * 10 if prefixe.isdigit() else 99
                
                # Déterminer la nature selon la classe
                if prefixe == "1":
                    nature = "Passif"
                elif prefixe in ["2", "3", "4", "5"]:
                    nature = "Actif"
                elif prefixe in ["6", "7", "8"]:
                    nature = "Compte de résultat"
                else:
                    nature = "Autre"
                
                compte_info = {
                    "grouping": grouping_num,
                    "nature": nature,
                    "ref": f"AUTO{prefixe}",
                    "libelle": f"Compte {numero}",
                    "libelle_groupe": f"AUTRES COMPTES - CLASSE {prefixe}"
                }
            
            grouping_num = compte_info.get("grouping")
            
            # Initialiser le grouping si nécessaire
            if grouping_num not in grouping_aggregation:
                # Récupérer les infos du grand groupe
                grand_groupe_info = grands_groupes_mapping.get(str(grouping_num), {})
                
                grouping_aggregation[grouping_num] = {
                    "grouping": grouping_num,
                    "nature": compte_info.get("nature", ""),
                    "ref": grand_groupe_info.get("ref", compte_info.get("ref", "")),
                    "libelle": grand_groupe_info.get("libelle", compte_info.get("libelle_groupe", "")),
                    "solde_n": 0,
                    "solde_n1": 0,
                    "variation": 0,
                    "variation_percent": 0,
            ...
            # Agréger les soldes
            solde_n = float(item.get("solde_n", 0) or 0)
            solde_n1 = float(item.get("solde_n1", 0) or 0)
            
            grouping_aggregation[grouping_num]["solde_n"] += solde_n
            grouping_aggregation[grouping_num]["solde_n1"] += solde_n1
            
            # Ajouter le compte aux détails
            grouping_aggregation[grouping_num]["comptes_detaille"].append({
        ...
        # Afficher les comptes non mappés (pour debug)
        if comptes_non_mappes:
            print(f"⚠️  {len(comptes_non_mappes)} comptes non mappés (grouping auto): {', '.join(comptes_non_mappes[:10])}{'...' if len(comptes_non_mappes) > 10 else ''}")

        # =============================
        # 4️⃣ Calculer les variations et formater le résultat
        # =============================
        result = []
        
        for grouping_num, group_data in sorted(grouping_aggregation.items()):
            # Calculer la variation
            variation = group_data["solde_n"] - group_data["solde_n1"]
            variation_percent = 0
            if group_data["solde_n1"] != 0:
                variation_percent = (variation / abs(group_data["solde_n1"])) * 100
            
            group_data["variation"] = variation
            group_data["variation_percent"] = variation_percent
            
            # Trier les comptes détaillés par numéro
            group_data["comptes_detaille"].sort(key=lambda x: x.get("numero_compte", ""))
            
            # Ajouter 'comptes' comme alias de 'comptes_detaille' (compatibilité frontend)
            group_data["comptes"] = group_data["comptes_detaille"]
            
            result.append(group_data)
        
        print(f"✅ Grouping généré : {len(result)} grands groupes")
        
        # Afficher un résumé par nature
        nature_summary = {}
        for group in result:
            nature = group.get("nature", "Autre")
            nature_summary[nature] = nature_summary.get(nature, 0) + 1
        
        print(f"📊 Répartition par nature:")
        for nature, count in sorted(nature_summary.items()):
            print(f"   - {nature}: {count} groupes")
    ...
    def total_charges(self, id_mission):
        db = get_db()  # Obtenir la connexion à la base de données
        balance = db.Mission1.find_one({"_id": ObjectId(id_mission)})[
            "balance_variation"
        ]
        charges = sum(
            item["solde_n"]
            for item in balance
            if str(item["numero_compte"]).startswith("6")
        )
        return abs(charges)
    ...
    def get_benchmarks(self, id_mission):
        ...
        try:
            ...
            bench["profit_before_tax"] = int(efi["pnl"][33]["net_solde_n"]) + int(
                efi["pnl"][38]["net_solde_n"]
            )
    ...
    def save_materiality(self, id_mission, materialities):
        ...
        benchmark_exists = any(
            item.get("benchmark") == materialities["benchmark"]
            for item in existing_materiality
        )

        # Forcer des valeurs toujours positives pour les seuils (règle métier)
        try:
            materialities["materiality"] = abs(
                int(materialities.get("materiality", 0) or 0)
            )
            materialities["performance_materiality"] = abs(
                int(materialities.get("performance_materiality", 0) or 0)
            )
            materialities["trivial_misstatements"] = abs(
                int(materialities.get("trivial_misstatements", 0) or 0)
            )
        except Exception:
            pass
        ...
        if benchmark_exists:
            ...
            update = db.Mission1.update_one(
                {
                    "_id": ObjectId(id_mission),
                    "materiality.benchmark": materialities["benchmark"],
                },
            ...
            update = db.Mission1.update_one(
                query, {"$push": {"materiality": materialities}}
            )
            return update.modified_count
    ...
    def validate_materiality(self, id_mission, benchmark):
        ...
        exist_choice = next(
            (item for item in materialities if item.get("choice")), None
        )
        if not exist_choice:
        ...
        result = db.Mission1.update_one(
            {"_id": ObjectId(id_mission)}, {"$set": {"materiality": materialities}}
        )
        return result.modified_count

    def get_materiality(self, id_mission):
        db = get_db()  # Obtenir la connexion à la base de données
        materiality = db.Mission1.find_one(
            {"_id": ObjectId(id_mission)}, {"_id": 0, "materiality": 1}
        )
        return materiality
    ...
    def materialite(self, id_mission):
        ...
        try:
            ...
            if not materiality:
                print(
                    f"📊 Calcul des benchmarks de matérialité pour la mission {id_mission}"
                )
                ...
                if materiality:
                    db.Mission1.update_one(
                        {"_id": ObjectId(id_mission)},
                        {"$set": {"materiality": materiality}},
                    )
                    print(
                        f"✅ {len(materiality)} benchmark(s) sauvegardé(s) dans la base de données"
                    )
            ...
            if not materiality:
                ...
                elif not isinstance(efi, dict) or not any(
                    section in efi for section in ["actif", "passif", "pnl"]
                ):
            ...
            return {
                "ok": False,
                "message": f"Erreur lors du calcul de la matérialité: {str(e)}",
                "materiality": [],
            }
    ...
    def _calculate_materiality_benchmarks(self, mission):
        ...
        try:
            ...
            if not efi:
                ...
                print(
                    "💡 Conseil: Veuillez générer les états financiers préliminaires (Étape 5) avant de calculer les matérialités"
                )
                return []
            ...
            if profit_before_tax and profit_before_tax != 0:
                ...
                print(
                    f"⚠️ Valeur profit_before_tax non trouvée ou nulle: {profit_before_tax}"
                )
            ...
            if (
                total_assets and total_assets > 0
            ):  # Les actifs ne peuvent pas être négatifs
                # Multiplier par -1 car les débits sont en + et les crédits en - dans la Balance Générale
            ...
            if (
                total_expenses and total_expenses > 0
            ):  # Les charges doivent toujours être positives
                # Multiplier par -1 car les débits sont en + et les crédits en - dans la Balance Générale
            ...
            if benchmarks:
                print(
                    f"✅ {len(benchmarks)} benchmark(s) calculé(s) avec succès: {valeurs_trouvees}"
                )
            else:
                print(
                    "❌ Aucun benchmark n'a pu être calculé. Vérifiez que les états financiers contiennent les données nécessaires."
                )
                print(
                    "💡 Les valeurs recherchées sont: profit_before_tax, ebitda, revenue, total_assets, total_expenses"
                )
    ...
    def _get_financial_value(self, efi, key):
        ...
        try:
            ...
            if key == "total_assets" and "actif" in efi:
                ...
                total = sum(
                    item.get("net_solde_n", 0)
                    for item in efi.get("actif", [])
                    if item.get("net_solde_n", 0)
                )
    ...
    def make_quantitative_analysis(self, id_mission):
        ...
        materiality = next(
            (mat for mat in mission["materiality"] if mat.get("choice")), None
        )
        ...
        if materiality is not None:
            ...
            result = db.Mission1.update_one(
                {"_id": ObjectId(id_mission)}, {"$set": {"grouping": grouping}}
            )
            return result.modified_count
        else:
            return 0
    ...
    def make_qualitative_analysis(self, id_mission, significant):
        ...
        for item in grouping:
            value = next(
                (
                    elt["significant"]
                    for elt in significant
                    if elt.get("compte") == item.get("compte")
                ),
                None,
            )
            if value is None:
    ...
    # ---------- Récup mission ----------
    @staticmethod
    def afficher_informations_missions(id_client):
        db = get_db()  # Obtenir la connexion à la base de données
        _id = id_client
    ...
    def prod_efi(self, balance_n, balance_n1, balance_variation):
        ...
        for mapp in mapping:
            mapp["brut_cpt"] = (
                mapp["brut_cpt"].split(",")
                if mapp.get("brut_cpt") is not None
                else mapp.get("brut_cpt")
            )
            mapp["amor_cpt"] = (
                mapp["amor_cpt"].split(",")
                if mapp.get("amor_cpt") is not None
                else mapp.get("amor_cpt")
            )
            mapp["net_cpt"] = (
                mapp["net_cpt"].split(",")
                if mapp.get("net_cpt") is not None
                else mapp.get("net_cpt")
            )
            mapp["brut_except_cpt"] = (
                mapp["brut_except_cpt"].split(",")
                if mapp.get("brut_except_cpt") is not None
                else mapp.get("brut_except_cpt", [])
            )
            mapp["amor_except_cpt"] = (
                mapp["amor_except_cpt"].split(",")
                if mapp.get("amor_except_cpt") is not None
                else mapp.get("amor_except_cpt", [])
            )
            mapp["net_except_cpt"] = (
                mapp["net_except_cpt"].split(",")
                if mapp.get("net_except_cpt") is not None
                else mapp.get("net_except_cpt", [])
            )
        ...
        for efi in list_efi:
            ...
            for data in select_mapping:
                ...
                if data.get("brut_cpt") and data.get("amor_cpt"):
                    ...
                    brut_solde_n = sum(
                        item["solde_reel"]
                        for item in balance_n
                        if any(
                            str(item["numero_compte"]).startswith(cpt)
                            for cpt in data["brut_cpt"]
                        )
                    )
                    amor_solde_n = sum(
                        item["solde_reel"]
                    ...
                    brut_except_n = sum(
                        ...
                        if any(
                            str(item["numero_compte"]).startswith(cpt)
                            for cpt in data.get("brut_except_cpt", [])
                        )
                    )
                    amor_except_n = sum(
                        item["solde_reel"]
                        ...
                        if any(
                            str(item["numero_compte"]).startswith(cpt)
                            for cpt in data.get("amor_except_cpt", [])
                        )
                    )
                    ...
                    net_except_n1 = sum(
                        ...
                        if any(
                            str(item["numero_compte"]).startswith(cpt)
                            for cpt in data.get("net_except_cpt", [])
                        )
                    )
                ...
                row["compte_to_be_used"] = (
                    str(data.get("brut_cpt"))
                    + str(data.get("amor_cpt"))
                    + str(data.get("net_cpt"))
                    + str(data.get("brut_except_cpt"))
                    + str(data.get("amor_except_cpt"))
                    + str(data.get("net_except_cpt"))
                )
                ...
                row["compte_to_be_used_off"] = list(
    ...
    def extract_grouping(self, id_mission):
        ...
        grouping = mission["grouping"]
        materiality = next(
            item for item in mission["materiality"] if item["choice"] is True
        )
        ...
        for iteration, data in enumerate(balances):
            ...
            sheet["G" + new_iteration] = next(
                item["significant"]
                for item in grouping
                if item["compte"] == value_grouping
            )
            sheet["H" + new_iteration] = next(
                item["materiality"]
                for item in grouping
                if item["compte"] == value_grouping
            )
            sheet["I" + new_iteration] = next(
                item["mat_sign"]
                for item in grouping
                if item["compte"] == value_grouping
            )
    ...
    def _get_sens_attendu_compte(self, numero_compte):
        ...
        if (
            prefixe.startswith("10")
            or prefixe.startswith("11")
            or prefixe.startswith("12")
            or prefixe.startswith("13")
            or prefixe.startswith("14")
            or prefixe.startswith("15")
            or prefixe.startswith("16")
            or prefixe.startswith("17")
            or prefixe.startswith("18")
            or prefixe.startswith("19")
        ):
            # Exceptions :
            ...
            if (
                numero_compte.startswith("105")
                or numero_compte.startswith("12")
                or numero_compte.startswith("109")
                or numero_compte.startswith("129")
            ):
        ...
        if prefixe.startswith("4"):
            ...
            elif prefixe.startswith("46") or prefixe.startswith(
                "47"
    ...
    def _comptes_doivent_etre_soldes(self, numero_compte):
        ...
        comptes_critiques = {
            "471": (
                "CRITIQUE",
                "Compte transitoire ou d'attente (471) doit être soldé en fin d'exercice. Tout solde indique des écritures en suspens non régularisées. Analyser ligne par ligne et régulariser chaque écriture.",
            ),
            "58": (
                "CRITIQUE",
                "Virements internes (58) doivent être soldés immédiatement. Utilisé pour les virements entre banques/caisses. Tout solde indique une erreur de lettrage ou un virement non comptabilisé des deux côtés.",
            ),
        }
        ...
        comptes_moyens_fin_exercice = {
            "109": (
                "MOYENNE",
                "Associés - capital souscrit non appelé (109) doit être soldé si tout le capital a été appelé. Si non soldé, vérifier la cohérence avec le capital social.",
            ),
            "475": (
                "MOYENNE",
                "Comptes de régularisation divers (475) doivent être soldés si toutes les régularisations ont été effectuées. Analyser les soldes résiduels.",
            ),
        }
        ...
        comptes_moyens_cours = {
            "422": (
                "MOYENNE",
                "Personnel - Rémunérations dues (422) doivent être soldées lors de la paie suivante. Des soldes anciens peuvent indiquer des erreurs de lettrage ou des paiements non régularisés.",
            ),
            "467": (
                "MOYENNE",
                "Autres comptes débiteurs/créditeurs (467) - compte de passage temporaire. Ne doit pas avoir de solde ancien. Vérifier l'ancienneté des montants non soldés.",
            ),
            "468": (
                "MOYENNE",
                "Charges à payer et produits à recevoir (468) doivent être soldés lors du paiement/encaissement effectif. Vérifier que les charges/produits provisionnés sont bien régularisés.",
            ),
            "4387": (
                "MOYENNE",
                "Organismes sociaux - Charges à payer (4387) doivent être soldées lors de la déclaration sociale suivante. Vérifier que les provisions sont bien régularisées.",
            ),
            "4386": (
                "MOYENNE",
                "Organismes sociaux - Autres charges à payer (4386) doivent être soldées lors du paiement effectif.",
            ),
            "431": (
                "MOYENNE",
                "Sécurité sociale (431) doit normalement être soldé chaque mois/trimestre lors du paiement. Un solde ancien indique un retard ou une erreur.",
            ),
            "437": (
                "MOYENNE",
                "Autres organismes sociaux (437) - même logique que 431. Normalement soldé chaque mois/trimestre lors du paiement.",
            ),
        }
        ...
        comptes_tva = {
            "44551": (
                "CRITIQUE",
                "TVA à décaisser (44551) doit être soldée lors du paiement de la TVA. Un solde ancien indique une TVA non payée → risque fiscal.",
            ),
            "44567": (
                "MOYENNE",
                "Crédit de TVA à reporter (44567) doit être soldé lorsque le crédit est imputé ou remboursé. Vérifier la cohérence avec les déclarations.",
            ),
            "44558": (
                "MOYENNE",
                "TVA à régulariser ou en attente (44558) - compte temporaire à solder rapidement. Tout solde doit être justifié précisément.",
            ),
        }
                ...
                if erreur.get("type") == "signe" and erreur.get("numero_compte"):
                    ...
                    if numero_compte and numero_compte[0].isdigit():
                        ...
                        try:
                            ...
                            if (
                                "devrait être" in message_original
                                and "mais le solde est" in message_original
                            ):
                                # Extraire le sens attendu
                                if "devrait être" in message_original:
                                    partie_attendu = message_original.split(
                                        "devrait être"
                                    )[1]
                                    sens_attendu = (
                                        partie_attendu.split(",")[0].strip()
                                        if "," in partie_attendu
                                        else partie_attendu.split(".")[0].strip()
                                    )

                                # Extraire le sens actuel
                                if "mais le solde est" in message_original:
                                    partie_actuel = message_original.split(
                                        "mais le solde est"
                                    )[1]
                                ...
                                if (
                                    "(" in message_original
                                    and "FCFA" in message_original
                                ):
                                    try:
                                        montant_part = message_original.split("(")[
                                            1
                                        ].split(")")[0]
                                ...
                                if montant:
                                    message_detaille += (
                                        f" Le solde anormal s'élève à {montant}."
                                    )
                        ...
                        anomalies_par_classe[classe].append(
                            {"compte": numero_compte, "message": message_detaille}
                        )
        structure = {
            "resume": {
    ...
    def _verifier_presence_comptes_obligatoires(self, numeros_comptes_presents):
        ...
        for classe, infos in classes_attendues.items():
            ...
            if comptes_classe:
                ...
                comptes_inexistants = infos["description"]
                details = f"{infos['libelle']} absente. Exemples attendus : {', '.join(infos['exemples'])}."
                status = "Absente"
                message = (
    ...
    def _valider_numerotation_syscohada(self, numero_compte):
        ...
        if not numero_clean:
            return (
                False,
                f"Le numéro de compte '{numero_compte}' ne contient aucun chiffre valide",
            )
        ...
        if len(numero_clean) < 2:
            return (
                False,
                f"Le numéro de compte '{numero_compte}' est trop court (minimum 2 chiffres requis)",
            )
        ...
        if premiere_classe not in ["1", "2", "3", "4", "5", "6", "7", "8", "9"]:
            return (
                False,
                f"Le numéro de compte '{numero_compte}' commence par '{premiere_classe}' qui n'est pas une classe valide (1-9 selon SYSCOHADA)",
            )
        ...
        if len(numero_clean) >= 2:
            ...
            try:
                ...
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' contient des caractères non numériques invalides",
                )
            ...
            if classe == "1" and not (10 <= deux_premiers_int <= 19):
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' de la classe 1 doit commencer par 10-19 selon SYSCOHADA",
                )
            ...
            if classe == "2" and not (20 <= deux_premiers_int <= 29):
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' de la classe 2 doit commencer par 20-29 selon SYSCOHADA",
                )
            ...
            if classe == "3" and not (30 <= deux_premiers_int <= 39):
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' de la classe 3 doit commencer par 30-39 selon SYSCOHADA",
                )
            ...
            if classe == "4" and not (40 <= deux_premiers_int <= 49):
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' de la classe 4 doit commencer par 40-49 selon SYSCOHADA",
                )
            ...
            if classe == "5" and not (50 <= deux_premiers_int <= 59):
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' de la classe 5 doit commencer par 50-59 selon SYSCOHADA",
                )
            ...
            if classe == "6" and not (60 <= deux_premiers_int <= 69):
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' de la classe 6 doit commencer par 60-69 selon SYSCOHADA",
                )
            ...
            if classe == "7" and not (70 <= deux_premiers_int <= 79):
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' de la classe 7 doit commencer par 70-79 selon SYSCOHADA",
                )
            ...
            if classe == "8" and not (80 <= deux_premiers_int <= 89):
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' de la classe 8 doit commencer par 80-89 selon SYSCOHADA",
                )
            ...
            if classe == "9" and not (90 <= deux_premiers_int <= 99):
                return (
                    False,
                    f"Le numéro de compte '{numero_compte}' de la classe 9 doit commencer par 90-99 selon SYSCOHADA",
                )
    ...
    def _coherence_checks_for_year(self, lines):
        ...
        comptes_dupliques = []
        numeros_comptes_valides = (
            []
        )  # Pour vérifier la présence des comptes obligatoires
        ...
        nb_comptes_analyses = len(
            [x for x in lines if x]
        )  # Toutes les lignes non vides
        ...
        if len(lines) > 0:
            print(
                f"[PREMIER CONTRÔLE] Exemple de données (5 premières lignes avec valeurs) :"
            )
            count = 0
            for i, x in enumerate(lines):
                ...
                if int(deb) > 0 or int(cre) > 0:
                    print(
                        f"  Ligne {i+1}: compte={num}, libelle={lib}, debit_fin={deb:,}, credit_fin={cre:,}, solde={abs(int(deb)-int(cre)):,}"
                    )
                    count += 1
        ...
        if sum_deb_fin != sum_cre_fin:
            ...
            for x in lines:
                ...
                if (
                    numero_compte
                    and numero_compte != "None"
                    and numero_compte.lower() != "nan"
                ):
            ...
            print(
                f"[PREMIER CONTRÔLE] Erreur d'équilibre ajoutée : type={erreur_equilibre['type']}, message_length={len(message)}"
            )
            ...
            print(
                f"[PREMIER CONTRÔLE] Rapport mis à jour : equilibre_global=False, ecart={ecart:,} FCFA, total_debits={sum_deb_fin:,} FCFA, total_credits={sum_cre_fin:,} FCFA"
            )
        else:
            # Les totaux sont égaux - équilibre OK
            report["equilibre_global"] = True
            print(
                f"[PREMIER CONTRÔLE] ✅ Équilibre OK : Total débits = Total crédits = {sum_deb_fin:,} FCFA"
            )
        ...
        print(
            f"[PREMIER CONTRÔLE ARITHMÉTIQUE] Exécuté : {nb_comptes_analyses} ligne(s) analysée(s), Total débits = {sum_deb_fin:,} FCFA, Total crédits = {sum_cre_fin:,} FCFA, Équilibre = {'OK' if report.get('equilibre_global', True) else 'DÉSÉQUILIBRÉ'}"
        )
        ...
        print(
            f"[SECOND CONTRÔLE] Début de la vérification de la formule pour {len(lines)} ligne(s) de balance"
        )
        ...
        for idx, x in enumerate(lines):
            ...
            if (
                not numero_compte
                or numero_compte == "None"
                or numero_compte.lower() == "nan"
            ):
                continue
        ...
        print(
            f"[SECOND CONTRÔLE] Début de la deuxième passe : vérification de la formule pour tous les comptes"
        )
        for idx, x in enumerate(lines):
            numero_compte = str(x.get("numero_compte", "")).strip()
            if (
            ...
            if numero_compte == "46210010":
                print(
                    f"[SECOND CONTRÔLE] ⚠️  COMPTE 46210010 DÉTECTÉ - Début de la vérification"
                )
            ...
            if debit_explicite_present:
                mouvement_debit_explicite = (
                    x.get("mouvement_debit") or x.get("debit_mvt") or 0
                )
                try:
            ...
            if credit_explicite_present:
                mouvement_credit_explicite = (
                    x.get("mouvement_credit") or x.get("credit_mvt") or 0
                )
                try:
            ...
            if ecart > 0:
                ...
                print(
                    f"  Solde initial: {solde_initial:,} (Débit init {di:,} - Crédit init {ci:,})"
                )
                print(
                    f"  Mouvements: Débit {mouvement_debit:,} | Crédit {mouvement_credit:,} | Net {mouvement_debit - mouvement_credit:,} (source: {source_mouvements})"
                )
                print(f"  Solde attendu: {solde_cloture_attendu:,} FCFA")
                print(
                    f"  Solde réel: {solde_fin:,} FCFA (Débit fin {df:,} - Crédit fin {cf:,})"
                )
            ...
            if ecart > 0:
                ...
                print(
                    f"[SECOND CONTRÔLE] Erreur détectée pour compte {numero_compte}: écart={ecart:,} FCFA (solde_fin={solde_fin:,}, attendu={solde_cloture_attendu:,})"
                )
                ...
                compte_display = (
                    "[SANS NUMÉRO]"
                    if numero_compte.startswith("LIGNE_SANS_NUMERO_")
                    else numero_compte
                )
                ...
                if numero_compte == "46210010":
                    print(
                        f"[SECOND CONTRÔLE] Compte 46210010: Formule respectée (écart={ecart:.2f} <= tolérance 0.01)"
                    )
            ...
            doit_etre_solde, gravite, message_solde = self._comptes_doivent_etre_soldes(
                numero_compte
            )
        ...
        if nb_comptes_verifies_formule > 0:
            ...
            print(
                f"[SECOND CONTRÔLE] Rapport créé : statut={statut_formule}, vérifiés={nb_comptes_verifies_formule}, OK={nb_comptes_formule_ok}, Erreurs={nb_comptes_formule_erreur}"
            )
        ...
        if nb_comptes_verifies_vraisemblance > 0:
            ...
            print(
                f"[CONTRÔLE VRAISEMBLANCE] Rapport créé pour cette balance : statut={statut_vraisemblance}, vérifiés={nb_comptes_verifies_vraisemblance}, OK={nb_comptes_vraisemblance_ok}, Erreurs signe={nb_comptes_erreur_signe}, Erreurs soldes={nb_comptes_erreur_soldes}"
            )
            print(
                f"[CONTRÔLE VRAISEMBLANCE] ⚠️ NOTE : La structure (tableaux des classes, comptes à solder) est identique pour toutes les balances car ce sont des règles générales SYSCOHADA."
            )
            print(
                f"[CONTRÔLE VRAISEMBLANCE] ⚠️ Les statistiques ci-dessus sont spécifiques à cette balance et peuvent différer d'une balance à l'autre."
            )
        ...
        report["totaux"] = {
            ...
            "nb_erreurs_total": nb_erreurs_identite
            + nb_erreurs_signe
            + nb_erreurs_arithmetique
            + nb_erreurs_comptes_soldes,
    ...
    @staticmethod
    def controle_coherence(id_mission):
        ...
        for idx, bal_id in enumerate(bal_ids):
            ...
            resultats.append({"annee": annee, "rapport": rapport})

        payload = {"ok": True, "data": resultats}

        local_db.Mission1.update_one(
            {"_id": ObjectId(id_mission)}, {"$set": {"controle_coherence": payload}}
        )
    ...
    def _index_by_compte(self, lines):
        ...
        for idx, x in enumerate(lines):
            ...
            if idx < 3:
                print(
                    f"  📋 Ligne {idx}: numero_compte={num_compte} (type: {type(num_compte)}, bool: {bool(num_compte)})"
                )
        ...
        try:
            ...
            try:
                ...
                if local_db is None:
                    ...
                    print(
                        "⚠️ controle_intangibilite: get_db() a renvoyé None, tentative de reconnexion..."
                    )
                    ensure_connection()
                    local_db = get_db()
                if local_db is None:
                    raise RuntimeError(
                        "controle_intangibilite: Base de données non connectée (get_db a renvoyé None)"
                    )
            except Exception as e:
                print(
                    f"❌ controle_intangibilite: impossible d'obtenir la connexion DB: {e}"
                )
                raise
            ...
            try:
                mission = local_db.Mission1.find_one({"_id": ObjectId(id_mission)})
                print(
                    f"🔍 Recherche avec ObjectId: Mission trouvée = {mission is not None}"
                )
            except Exception as e:
                ...
                try:
                    ...
                    print(
                        f"🔍 Recherche avec ID string: Mission trouvée = {mission is not None}"
                    )
            ...
            if not mission:
                print(
                    f"[CONTROLE_INTANGIBILITE] ERREUR: Mission non trouvee avec ID: {id_mission}"
                )
                ...
                try:
                    ...
                    if sample_missions:
                        ...
                        for m in sample_missions:
                            print(
                                f"   - ID: {m['_id']}, id_client: {m.get('id_client', 'N/A')}, année: {m.get('annee_auditee', 'N/A')}"
                            )
                    else:
            ...
            if len(bal_ids) < 2:
                print(
                    f"[CONTROLE_INTANGIBILITE] ERREUR: Pas assez de balances ({len(bal_ids)})"
                )
                return {
                    "ok": False,
                    "message": "Il faut au moins N et N-1.",
                    "comptes": [],
                }
            ...
            annee_auditee = (
                int(mission.get("annee_auditee", 0))
                if mission.get("annee_auditee")
                else 0
            )
            ...
            for bal_id in bal_ids:
                ...
                if bal_doc:
                    ...
                    annee_balance = (
                        bal_doc.get("annee_balance") or bal_doc.get("periode") or ""
                    )
                    balance_docs.append(
                        {
                            "id": bal_id,
            ...
            periode_N = (
                str(balance_docs[-1]["annee"]) if balance_docs[-1]["annee"] else "N"
            )
            periode_N1 = (
                str(balance_docs[-2]["annee"]) if balance_docs[-2]["annee"] else "N-1"
            )
            ...
            if not bal_N or len(bal_N) == 0:
                return {
                    "ok": False,
                    "message": f"La balance N ({periode_N}) ne contient aucune donnée.",
            ...
            if not bal_N1 or len(bal_N1) == 0:
                return {
                    "ok": False,
                    "message": f"La balance N-1 ({periode_N1}) ne contient aucune donnée.",
                    "periodes": {"N": periode_N, "N-1": periode_N1},
                    "comptes": [],
                }
            ...
            if len(bal_N) > 0:
                print(
                    f"📋 Exemple ligne N (première): {list(bal_N[0].keys()) if bal_N[0] else 'vide'}"
                )
                if bal_N[0] and "numero_compte" in bal_N[0]:
                    num_compte = bal_N[0].get("numero_compte")
                    print(
                        f"📋 Premier numero_compte N: '{num_compte}' (type: {type(num_compte)}, value: {repr(num_compte)})"
                    )
                else:
                    print(
                        f"📋 Première ligne N ne contient pas 'numero_compte': {bal_N[0].keys() if bal_N[0] else 'ligne vide'}"
                    )
            ...
            if len(bal_N1) > 0:
                if bal_N1[0] and "numero_compte" in bal_N1[0]:
                    ...
                    print(
                        f"📋 Premier numero_compte N-1: '{num_compte}' (type: {type(num_compte)}, value: {repr(num_compte)})"
                    )

            print(f"[CONTROLE_INTANGIBILITE] Indexation des comptes...")
            print(
                f"[CONTROLE_INTANGIBILITE] Avant indexation: bal_N contient {len(bal_N)} lignes, bal_N1 contient {len(bal_N1)} lignes"
            )
            ...
            print(
                f"[CONTROLE_INTANGIBILITE]    - Comptes indexes dans N-1: {len(idxN1)}"
            )
            print(
                f"[CONTROLE_INTANGIBILITE] idxN type: {type(idxN)}, idxN1 type: {type(idxN1)}"
            )
            ...
            if len(bal_N) > 0 and len(idxN) == 0:
                print(
                    f"⚠️  PROBLÈME: bal_N contient {len(bal_N)} lignes mais idxN est vide!"
                )
                ...
                if len(bal_N) > 0 and isinstance(bal_N[0], dict):
                    ...
                    print(
                        f"   - numero_compte de la première ligne: {bal_N[0].get('numero_compte')}"
                    )

            if len(bal_N1) > 0 and len(idxN1) == 0:
                print(
                    f"⚠️  PROBLÈME: bal_N1 contient {len(bal_N1)} lignes mais idxN1 est vide!"
                )
                ...
                if len(bal_N1) > 0 and isinstance(bal_N1[0], dict):
                    ...
                    print(
                        f"   - numero_compte de la première ligne: {bal_N1[0].get('numero_compte')}"
                    )
            ...
            if len(idxN) == 0 and len(idxN1) == 0:
                ...
                print(
                    f"   - Balance N: {len(bal_N)} lignes brutes, mais 0 comptes indexés"
                )
                print(
                    f"   - Balance N-1: {len(bal_N1)} lignes brutes, mais 0 comptes indexés"
                )
                print(
                    f"   - Vérifiez que les lignes contiennent bien un champ 'numero_compte' avec une valeur non vide"
                )
                print(
                    f"   - DEBUG: Type de bal_N[0] si existe: {type(bal_N[0]) if bal_N and len(bal_N) > 0 else 'N/A'}"
                )
                print(
                    f"   - DEBUG: Type de bal_N1[0] si existe: {type(bal_N1[0]) if bal_N1 and len(bal_N1) > 0 else 'N/A'}"
                )
                ...
                if len(bal_N) > 0:
                    ...
                    if "numero_compte" not in premiere_ligne:
                        print(
                            f"   - ⚠️  Le champ 'numero_compte' est absent de la première ligne"
                        )
            ...
            print(
                f"🔍 Début du traitement: {len(idxN)} comptes en N, {len(idxN1)} comptes en N-1"
            )

            if len(idxN) == 0:
                print(
                    f"⚠️  ATTENTION: idxN est vide même si bal_N contient {len(bal_N)} lignes"
                )

            if len(idxN1) == 0:
                print(
                    f"⚠️  ATTENTION: idxN1 est vide même si bal_N1 contient {len(bal_N1)} lignes"
                )
                ...
                try:
                    ...
                    if prev:
                        ...
                        if len(tous_comptes) < 5:
                            print(
                                f"  📋 Compte {num}: ouvN={ouvN} (di={di}-ci={ci}), clotN1={clotN1} (df={df}-cf={cf}), ecart={ecart} (ouvN - clotN1)"
                            )
                        ...
                        tous_comptes.append(
                            {
                                ...
                                "message": (
                                    f"Ouverture N {ouvN} ≠ Clôture N-1 {clotN1}"
                                    if ecart != 0
                                    else f"Ouverture N {ouvN} = Clôture N-1 {clotN1}"
                                ),
                                "justification": (
                                    f"Écart de {ecart} entre l'ouverture de l'exercice N ({ouvN}) et la clôture de l'exercice N-1 ({clotN1})."
                                    if ecart != 0
                                    else "Aucun écart détecté."
                                ),
                                "conclusion_audit": (
                                    "Écart significatif détecté - Nécessite une justification et une documentation des causes de cette variation."
                                    if ecart != 0
            ...
            print(
                f"📊 Après traitement des comptes N: {comptes_ajoutes} comptes ajoutés, {comptes_erreur} erreurs"
            )
            ...
            print(
                f"📊 Après traitement des comptes N-1: {len(tous_comptes)} comptes au total"
            )
            ...
            ecarts_count = len(
                [
                    ...
                    if c["status"] in ["ecart", "nouveau", "supprime"]
                ]
            )

            print(f"📊 Résumé final: {len(tous_comptes)} comptes traités")
            print(
                f"   - Comptes OK: {len([c for c in tous_comptes if c['status'] == 'ok'])}"
            )
            print(
                f"   - Comptes avec écart: {len([c for c in tous_comptes if c['status'] == 'ecart'])}"
            )
            print(
                f"   - Comptes nouveaux: {len([c for c in tous_comptes if c['status'] == 'nouveau'])}"
            )
            print(
                f"   - Comptes supprimés: {len([c for c in tous_comptes if c['status'] == 'supprime'])}"
            )
            ...
            print(
                f"✅ Rapport généré: {len(tous_comptes)} comptes, {ecarts_count} écarts"
            )
            print(f"📋 Clés du rapport: {list(report.keys())}")
            print(
                f"📋 Nombre de comptes dans le rapport: {len(report.get('comptes', []))}"
            )
            ...
            if len(tous_comptes) == 0:
                ...
                print(
                    f"   - Vérifiez les logs ci-dessus pour comprendre pourquoi tous_comptes est vide"
                )
            ...
            local_db.Mission1.update_one(
                {"_id": ObjectId(id_mission)},
                {"$set": {"controle_intangibilite": report}},
            )
            print(
                f"[CONTROLE_INTANGIBILITE] Rapport sauvegarde dans la base de donnees"
            )
            print(
                f"[CONTROLE_INTANGIBILITE] Rapport retourne: total_comptes={report.get('total_comptes')}, ecarts={report.get('ecarts_count')}"
            )
            ...
            return {
                "ok": False,
                "message": f"Erreur lors du contrôle d'intangibilité: {str(e)}",
                "comptes": [],
            }
    ...
    def ajouter_reclassement_intangibilite(self, id_mission, reclassement_data):
        ...
        try:
            ...
            try:
                ...
                if local_db is None:
                    raise RuntimeError(
                        "ajouter_reclassement_intangibilite: Base de données non connectée"
                    )
            except Exception as e:
                print(
                    f"❌ ajouter_reclassement_intangibilite: impossible d'obtenir la connexion DB: {e}"
                )
                raise
            ...
            reclassement = {
                ...
                "compte_destination": str(
                    reclassement_data["compte_destination"]
                ).strip(),
                "montant": montant,
                "sens": reclassement_data["sens"],
                "justification": reclassement_data["justification"],
                "date_reclassement": reclassement_data.get(
                    "date_reclassement", datetime.datetime.now().strftime("%Y-%m-%d")
                ),
            ...
            print(
                f"[RECLASSEMENT] Reclassement ajouté avec succès: {reclassement['id']}"
            )
            ...
            return {
                "ok": False,
                "message": f"Erreur lors de l'ajout du reclassement: {str(e)}",
            }
    ...
    def supprimer_reclassement_intangibilite(self, id_mission, reclassement_id):
        ...
        try:
            print(
                f"[RECLASSEMENT] Suppression du reclassement {reclassement_id} pour mission {id_mission}"
            )
            ...
            try:
                ...
                if local_db is None:
                    raise RuntimeError(
                        "supprimer_reclassement_intangibilite: Base de données non connectée"
                    )
            except Exception as e:
                print(
                    f"❌ supprimer_reclassement_intangibilite: impossible d'obtenir la connexion DB: {e}"
                )
                raise
            ...
            reclassements_filtres = [
                r for r in reclassements if r.get("id") != reclassement_id
            ]
    ...
    def lister_reclassements_intangibilite(self, id_mission):
        ...
        try:
            ...
            try:
                ...
                if local_db is None:
                    raise RuntimeError(
                        "lister_reclassements_intangibilite: Base de données non connectée"
                    )
            except Exception as e:
                print(
                    f"❌ lister_reclassements_intangibilite: impossible d'obtenir la connexion DB: {e}"
                )
                raise
            ...
            reclassements = mission.get("reclassements_intangibilite", [])
            reclassements_actifs = [
                r for r in reclassements if r.get("statut", "actif") == "actif"
            ]
    ...
    @staticmethod
    def controle_intangibilite_avec_reclassements(id_mission):
        """
        ...
        try:
            ...
            print(
                f"[CONTROLE_AVEC_RECLASSEMENTS] {len(reclassements)} reclassement(s) à appliquer"
            )
            ...
            for compte in rapport_standard.get("comptes", []):
                ...
                for reclassement in reclassements:
                    if (
                        reclassement["compte_source"] == numero_compte
                        or reclassement["compte_destination"] == numero_compte
                    ):
                        ...
                        # Recalculer l'écart
                        if (
                            compte_ajuste["ouverture_n"] is not None
                            and compte_ajuste["cloture_n1"] is not None
                        ):
                            compte_ajuste["ecart"] = (
                                compte_ajuste["cloture_n1"]
                                - compte_ajuste["ouverture_n"]
                            )

                        # Mettre à jour le statut
                        if (
                            abs(compte_ajuste["ecart"]) < 0.01
                            ...
                            compte_ajuste["conclusion_audit"] = (
                                "Écart justifié par reclassement documenté."
                            )
                        else:
                            compte_ajuste["status"] = "ecart_partiel"
                            compte_ajuste["message"] = (
                                f"Écart partiellement résolu par reclassement"
                            )
            ...
            ecarts_count = len(
                [
                    c
                    for c in comptes_ajustes
                    if c["status"] in ["ecart", "nouveau", "supprime", "ecart_partiel"]
                ]
            )
            ...
            print(
                f"[CONTROLE_AVEC_RECLASSEMENTS] Rapport final: {len(comptes_ajustes)} comptes, {ecarts_count} écarts restants"
            )
            ...
            return {
                "ok": False,
                "message": f"Erreur lors du contrôle avec reclassements: {str(e)}",
                "comptes": [],
            }
    ...
    def classement_bilan(self, id_mission):
        try:
            ...
            balance_n = db.Balance.find_one(
                {"_id": ObjectId(balances_ids[0])}, {"balance": 1}
            )
            balance_n1 = db.Balance.find_one(
                {"_id": ObjectId(balances_ids[1])}, {"balance": 1}
            )

            if not balance_n or not balance_n1:
                return {
                    "ok": False,
                    "message": "Balances introuvables",
                    "classement": [],
                }
            ...
            # 🔹 3. Index balance N-1
            balance_n1_index = {
                str(item.get("numero_compte")): item for item in balance_n1_data
            }
            ...
            if not prefixes:
                return {
                    "ok": False,
                    "message": "Aucun compte valide trouvé",
                    "classement": [],
                }
            ...
            for group in table_grouping:
                ...
                for item_n in balance_n_data:
                    ...
                    if numero.startswith(prefixe):
                        ...
                        solde_item_n = (item_n.get("debit_fin", 0) or 0) - (
                            item_n.get("credit_fin", 0) or 0
                        )
                        solde_item_n1 = (item_n1.get("debit_fin", 0) or 0) - (
                            item_n1.get("credit_fin", 0) or 0
                        )
            ...
            db.Mission1.update_one(
                {"_id": ObjectId(id_mission)}, {"$set": {"classement_bilan": report}}
            )
    ...
    def etats_financiers_preliminaires(self, id_mission):
        try:
            ...
            balance_n = local_db.Balance.find_one(
                {"_id": ObjectId(balances[0])}, {"balance": 1}
            )
            balance_n1 = local_db.Balance.find_one(
                {"_id": ObjectId(balances[1])}, {"balance": 1}
            )
            ...
            mapping_path = os.path.join(
                os.path.dirname(__file__), "..", "mapping_efi.json"
            )
            with open(mapping_path, "r", encoding="utf-8") as file:
            ...
            print(
                f"📊 États financiers générés - Actif: {len(efi_organized.get('actif', []))} lignes, Passif: {len(efi_organized.get('passif', []))} lignes, PNL: {len(efi_organized.get('pnl', []))} lignes"
            )
            ...
            local_db.Mission1.update_one(
                {"_id": ObjectId(id_mission)},
                {"$set": {"etats_financiers_preliminaires": report}},
            )
            return report
    ...
    def analyse_quantitative(self, id_mission):
        try:
            ...
            if not grouping:
                return {
                    "ok": False,
                    "message": "Données de grouping manquantes",
                    "analyse": [],
                }

            # Récupérer les données de matérialité
            materiality = mission.get("materiality", [])
            selected_materiality = next(
                (mat for mat in materiality if mat.get("choice")), None
            )

            if not selected_materiality:
                return {
                    "ok": False,
                    "message": "Aucune matérialité sélectionnée",
                    "analyse": [],
                }
            ...
            total_significant_amount = sum(
                [a["solde_n"] for a in analyse_data if a["is_significant"]]
            )
            ...
            local_db.Mission1.update_one(
                {"_id": ObjectId(id_mission)},
                {"$set": {"analyse_quantitative": report}},
            )
            return report
        ...
        try:
            ...
            for item in grouping:
                ...
                positive_responses = sum(
                    ...
                    if compte_responses.get(f"Q{q}", False)
                )
                ...
                if qualitative_score >= 50:
                    ...
                    is_qualitatively_significant = (
                        False  # À évaluer mais pas significatif
                    )
                else:
                ...
                analyse_data.append(
                    {
                        ...
                        "status": (
                            "À tester"
                            if is_qualitatively_significant
                            else "Ne pas tester"
                        ),
                    }
                )
            ...
            # Statistiques
            total_accounts = len(analyse_data)
            significant_accounts = len(
                [a for a in analyse_data if a["is_qualitatively_significant"]]
            )
            total_positive_responses = sum(
                [a["positive_responses"] for a in analyse_data]
            )
            ...
            report = {
                ...
                "statistics": {
                    ...
                    "average_score": (
                        sum([a["qualitative_score"] for a in analyse_data])
                        / total_accounts
                        if total_accounts > 0
                        else 0
                    ),
            ...
            # Sauvegarder l'analyse dans la mission
            local_db.Mission1.update_one(
                {"_id": ObjectId(id_mission)}, {"$set": {"analyse_qualitative": report}}
            )
            return report
        ...
        try:
            ...
            for item in grouping:
                ...
                final_status = self.determine_final_status(
                    is_quantitatively_significant, is_qualitatively_significant
                )
            ...
            synthese_data.sort(
                key=lambda x: risk_order.get(x["risk_level"], 0), reverse=True
            )

            # Statistiques
            total_accounts = len(synthese_data)
            accounts_to_test = len(
                [a for a in synthese_data if a["final_status"] == "À tester"]
            )
            accounts_not_to_test = len(
                [a for a in synthese_data if a["final_status"] == "Ne pas tester"]
            )
            ...
            report = {
                ...
                "statistics": {
                    ...
                    "materiality_threshold": materiality_threshold,
                    "total_amount_to_test": sum(
                        [
                            a["solde_n"]
                            for a in synthese_data
                            if a["final_status"] == "À tester"
                        ]
                    ),
                },
            }
            ...
            local_db.Mission1.update_one(
                {"_id": ObjectId(id_mission)},
                {"$set": {"synthese_comptes_significatifs": report}},
            )
            return report
    ...
    def determine_final_status(
        self, is_quantitatively_significant, is_qualitatively_significant
    ):
    ...
    def presentation_comptes_significatifs(self, id_mission):
        try:
            ...
            if not analyse_quantitative.get("analyse") or not analyse_qualitative.get(
                "analyse"
            ):
                return {
                    "ok": False,
                    "message": "Analyses quantitative et qualitative requises. Veuillez d'abord exécuter les étapes 7 et 8.",
                    "presentation": [],
                }
            ...
            presentation_data.sort(
                key=lambda x: (priorite_order.get(x["priorite"], 0), x["solde_n"]),
                reverse=True,
            )
            ...
            significant_accounts = len(
                [
                    a
                    for a in presentation_data
                    if a["significativite_status"].startswith("Significatif")
                ]
            )
            non_significant_accounts = total_accounts - significant_accounts
            high_priority_accounts = len(
                [a for a in presentation_data if a["priorite"] == "Haute"]
            )
            total_significant_amount = sum(
                [
                    a["solde_n"]
            ...
            local_db.Mission1.update_one(
                {"_id": ObjectId(id_mission)},
                {"$set": {"presentation_comptes_significatifs": report}},
            )
            return report
        ...
        try:
            ...
            for item in grouping:
                ...
                if final_status == "Non évalué":
                    ...
                    quant_data = quant_index.get(compte, {})
                    is_quantitatively_significant = quant_data.get(
            ...
            accounts_to_validate = len(
                [a for a in revue_data if a["final_status"].startswith("À tester")]
            )
            ...
            validation_percentage = (
                (accounts_validated / accounts_to_validate * 100)
                if accounts_to_validate > 0
                else 100
            )
            ...
            local_db.Mission1.update_one(
                {"_id": ObjectId(id_mission)},
                {"$set": {"revue_analytique_finale": report}},
            )
            return report
    ...
    def generate_automatic_comment(
        self,
        compte,
        solde_n,
        solde_n1,
    ...
    def prod_efi(self, balance_n, balance_n1, balance_variation):
        ...
        for efi in list_efi:
            ...
            # Log pour vérifier le nombre de lignes générées
            print(
                f"📊 Structure {efi} générée: {len(structure)} lignes sur {len(select_mapping)} attendues"
            )

            # Log des références pour vérifier
            refs = [row.get("ref", "") for row in structure]
            print(
                f"📊 Références {efi} ({len(refs)}): {', '.join(refs[:30])}{'...' if len(refs) > 30 else ''}"
            )
    ...
    def audit_trail(self, id_mission):
        ...
        try:
            ...
            if local_db is None:
                ...
                print(
                    "⚠️ audit_trail: get_db() a renvoyé None, tentative de reconnexion..."
                )
                ensure_connection()
                local_db = get_db()
            if local_db is None:
                raise RuntimeError(
                    "audit_trail: Base de données non connectée (get_db a renvoyé None)"
                )
        except Exception as e:
