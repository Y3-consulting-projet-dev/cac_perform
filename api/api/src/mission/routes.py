...
def update_commentaire_route(id_mission):
    ...
    try:
        ...
        cls = Mission()
        success = cls.update_commentaire_perso(
            id_mission, numero_compte, commentaire_perso
        )

        if success:
            return make_response(
                jsonify({"response": "Commentaire mis à jour avec succès"}), 200
            )
        else:
            return make_response(
                jsonify({"error": "Échec de la mise à jour du commentaire"}), 500
            )
...
def controle_coherence_route(id_mission):
    report = Mission.controle_coherence(id_mission)
...
def controle_intangibilite_route(id_mission):
    ...
    report = Mission.controle_intangibilite_avec_reclassements(id_mission)
    ...
    if isinstance(comptes, list):
        ecarts = [
            ...
            for c in comptes
            if c
            and c.get("status") in ("ecart", "nouveau", "supprime", "ecart_partiel")
        ]
        report["comptes"] = ecarts
        report["ecarts_only"] = True
    print(
        f"[ROUTE] Rapport retourne: total_comptes={report.get('total_comptes', 'N/A')}, comptes_length={len(report.get('comptes', []))}"
    )
...
def init_qualitative_responses_route(id_mission):
    try:
        ...
        if not mission:
            return make_response(
                jsonify({"response": {"ok": False, "message": "Mission non trouvée"}}),
                404,
            )
                ...
                jsonify(
                    {
                        "response": {
                            "ok": True,
                            "message": "Réponses qualitatives initialisées avec succès",
                        }
                    }
                ),
                200,
        ...
        return make_response(
            jsonify({"response": {"ok": False, "message": str(e)}}), 500
        )
...
def save_qualitative_responses_route(id_mission):
    try:
        ...
        if not data:
            return make_response(
                jsonify({"response": {"ok": False, "message": "Aucune donnée reçue"}}),
                400,
            )
        ...
        print(
            f"✅ Résultat de la mise à jour: {result.modified_count} documents modifiés"
        )
    ...
    try:
        ...
        if len(valid_files) < 2:
            return make_response(
                jsonify(
                    {
                        "error": f"Seulement {len(valid_files)} fichier(s) valide(s) reçu(s), 2 requis"
                    }
                ),
                400,
            )
        ...
        print(
            f"Données reçues: annee={annee_auditee}, client={id_client}, debut={date_debut}, fin={date_fin}"
        )
        ...
        if donnees:
            ...
            return make_response(
                jsonify({"error": "Erreur lors de la création de la mission"}), 500
            )
    ...
    try:
        ...
        for balance_id in balance_ids:
            try:
                ...
                print(
                    f"⚠️ Erreur lors de la suppression de la balance {balance_id}: {str(e)}"
                )
        ...
        if result.deleted_count > 0:
            print(
                f"✅ Mission {id_mission} supprimée avec succès ({balances_supprimees} balance(s) supprimée(s))"
            )
            return make_response(
                jsonify(
                    {
            ...
            return make_response(
                jsonify({"error": "Erreur lors de la suppression de la mission"}), 500
            )
...
def show_assign_info(_id):
    infos = Mission.afficher_informations_missions(_id)
...
def make_qualitative_analysis(id_mission):
    try:
                ...
                jsonify(
                    {
                        "response": {
                            "ok": False,
                            "message": "Aucune donnée de grouping fournie",
                        }
                    }
                ),
                400,
        ...
        for compte in _list_unique_compte:
            ...
            _list = [
                {"question": item["question"], "significant": item["significant"]}
                for item in listGrouping
                if item["compte"] == compte
            ]
            data["data"] = _list
                ...
                jsonify(
                    {
                        "response": {
                            "ok": True,
                            "message": "Analyse qualitative appliquée avec succès",
...
def get_grouping_with_qualitative(id_mission):
    try:
        ...
        significant_accounts = len(
            [item for item in grouping if item.get("significant", False)]
        )
        ...
        result = {
            ...
            "statistics": {
                ...
                "percentage_significant": (
                    (significant_accounts / total_accounts * 100)
                    if total_accounts > 0
                    else 0
                ),
            },
                ...
                jsonify(
                    {
                        "response": {
                            "ok": True,
                            "message": "Revue analytique sauvegardée avec succès",
                        }
                    }
                ),
                200,
...
def ajouter_reclassement_route(id_mission):
    ...
    try:
        ...
        if result.get("ok", False):
            ...
            return make_response(
                jsonify({"error": result.get("message", "Erreur inconnue")}), 400
            )
