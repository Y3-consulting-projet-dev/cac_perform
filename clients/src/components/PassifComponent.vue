...
// Ordre officiel SYSCOHADA Bilan Passif (page 1049)
const SYSCOHADA_ORDER_PASSIF = [
...
const rows = computed(() => {
  const byRef = {}
  for (const row of (props.efiPassif || [])) {
    if (row?.ref) byRef[row.ref] = row
  }
  const result = []
  for (const ref of SYSCOHADA_ORDER_PASSIF) {
    if (byRef[ref]) result.push({ ...byRef[ref], _isGroup: GROUP_REFS.has(ref) })
  }
  for (const row of (props.efiPassif || [])) {
    if (row?.ref && !SYSCOHADA_ORDER_PASSIF.includes(row.ref))
...
onMounted(() => {
  console.log('📊 PassifComponent - efiPassif:', props.efiPassif)
  console.log('  - Refs:', props.efiPassif?.map(x => x.ref))
})
</script>
      ...
      <tbody class="bg-white divide-y divide-gray-100">
        <tr v-if="!rows.length">
          <td colspan="11" class="px-6 py-8 text-center text-gray-400 text-sm">
            Aucune donnée disponible | efiPassif: {{ props.efiPassif ? 'défini' : 'non défini' }} | Longueur: {{ props.efiPassif?.length || 0 }}
          </td>
        </tr>

        <tr
          v-for="(data, index) in rows"
          :key="`passif-${index}-${data.ref}`"
          :class="[
            data._isGroup
